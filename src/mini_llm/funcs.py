...
def scaled_dot_product_attention_grouped(
    ...
    query = query.reshape(
        -1, query.shape[-3], query.shape[-2], query.shape[-1], stream=stream
    )
    key = key.reshape(-1, key.shape[-3], key.shape[-2], key.shape[-1], stream=stream)
    value = value.reshape(
        -1, value.shape[-3], value.shape[-2], value.shape[-1], stream=stream
    )
    B, H_q, L, E = query.shape
    _, H, S, _ = key.shape
    assert H_q % H == 0
    n_repeats = H_q // H
    query = query.reshape((B, H, n_repeats, L, E), stream=stream)
    key = key.reshape((B, H, 1, S, E), stream=stream)
    value = value.reshape((B, H, 1, S, E), stream=stream)
    scores = (
        mx.matmul(query, key.swapaxes(-2, -1, stream=stream), stream=stream) * factor
    )
    if mask is not None:
        mask = mask.reshape(
            -1, H, n_repeats, mask.shape[-2], mask.shape[-1], stream=stream
        )
        scores = scores + mask
